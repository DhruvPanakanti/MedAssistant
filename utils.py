...
def load_artifacts(disease_key):
    ...
    if disease_key not in _cache:
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not load the '{disease_key}' model ({e}). This is almost "
                f"always an installed-package version that doesn't match what the "
                f"model was pickled with (scikit-learn's internal Cython modules, "
                f"e.g. for GradientBoostingClassifier, aren't guaranteed pickle-"
                f"compatible across versions). Run `pip install -r requirements.txt` "
                f"in this venv to resync versions, then restart."
            ) from e
